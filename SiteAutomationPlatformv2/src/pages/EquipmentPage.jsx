...
function EquipmentPage() {
  ...
  useEffect(() => {
    ...
    console.log('💡 [EquipmentPage] Raw formData received:', validFormData);

    // Check for lighting contacteur fields
    const lightingFields = Object.keys(validFormData).filter(k =>
      k.includes('nb_contacteurs') || k.includes('ref_disjoncteur_contacteur')
    );
    console.log('💡 [EquipmentPage] Lighting contacteur fields found:', lightingFields);
    lightingFields.forEach(field => {
      console.log(`  - ${field}: ${validFormData[field]}`);
    });

    // Always use zone-aware grouping
    const grouped = groupDataByZones(validFormData, ['Aero', 'Clim', 'Rooftop', 'Eclairage']);

    console.log('💡 [EquipmentPage] Grouped data after groupDataByZones:', grouped);

    // Add images to grouped data (zone-aware filtering)
    Object.keys(grouped).forEach(cardKey => {
      const { type, zone } = parseCardKey(cardKey);

      // Handle Comptage cards: type is "Comptage_Aero" but SQL stores "Aero_comptage"
      const isComptage = type.startsWith('Comptage_');
      const baseType = isComptage ? type.replace('Comptage_', '') : type;
      const sqlType = isComptage ? `${baseType}_comptage` : type;

      const imagesForCard = validImages
        .filter(img => {
          // Match by type (handle both regular and comptage formats)
          const typeMatches = img.type?.toLowerCase() === sqlType.toLowerCase();
...
useEffect(() => {
  ...
  const handleCardChange = (key, updates) => {
    setData((prev) => {
      ...
      // 🧹 Unified cleanup for dynamic fields using field registry
      if (nextData.data) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('🔍 [DEBUG] Comptage payload before flatten:', {
        selectedComptage,
        payloadComptage,
      ...
      console.log('🔍 [DEBUG] Flattened comptage fields:',
...
return (
  <PageLayout title="Équipements du site">
  ...
  <Space direction="vertical" size="middle" style={{ width: '100%' }}>
    ...
    <Form layout="vertical" form={form} style={{ marginBottom: 0 }}>
      <Form.Item style={{ marginBottom: 16 }}>
        {selected.length === 0 ? (
          <Empty
            description="Aucune carte configurée. Cliquez sur 'Ajouter une zone' pour commencer."
                    ...
                    <Space direction="vertical" size={8} style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <Typography.Text strong style={{ fontSize: 16 }}>
                          <span style={{ marginRight: 8 }}>{typeIcons[type] || '📦'}</span>
                          {type}
                        </Typography.Text>
                        <Badge
...
{selected.length === 0 ? (
  <Empty description="Aucune sélection" style={{ marginTop: 6 }} />
) : (
  <div
    style={{
      marginTop: 32,
  ...
  buttons={[
    {
      ...
      children: isSubmitting ? 'Sauvegarde...' : '💾 Enregistrer les données'
    },
    {
      danger: true,
      onClick: () => {
        try {
          ...
          setCurrentComptageIndex(0);
          form.resetFields();
          message.info('Page réinitialisée');
